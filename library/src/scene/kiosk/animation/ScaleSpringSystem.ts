...
export class ScaleSpringSystem implements ISystem {
    ...
    static ensureInstance(): ScaleSpringSystem {
        if (ScaleSpringSystem.instance === undefined || ScaleSpringSystem.instance === null) {
    ...
    update(_deltaTime: number) {
        ...
        this.allSprings.entities.forEach((entity) => {
            ...
            let transform: Transform = entity.getComponent(Transform)
            if (transform === undefined || transform === null) {
                ...
                let spring = entity.getComponent(ScaleSpringComponent)

                // check for a sync request
                if (spring.sync && spring.targetScale !== undefined && spring.targetScale !== null) {
                    spring.sync = false
                ...
                let dif = spring.targetScale.subtract(transform.scale)
                ...
                transform.scale = transform.scale.add(spring.velocity.scale(_deltaTime))
