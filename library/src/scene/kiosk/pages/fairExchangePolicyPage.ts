...
export class FairExchangePolicyPage {
    ...
    constructor(_kiosk: Kiosk, _parent: Entity, _productData: any) {
        ...
        // Description
        this.DescriptionText = new TextShape("Boson Exchange Policies combine protocol variables and the underlying contractual agreement of an\nexchange into a standardized policy, ensuring fair terms and protection for both buyer and seller.")
        ...
        this.Description.addComponent(new Transform({
        ...
        this.InformationHeadingsText = new TextShape("Policy Name\n\nDispute Period\n\nEscalation Period\n\nRedeemable NFT Terms\n\nBuyer & Seller Agreement")
        ...
        this.InformationHeadings.addComponent(new Transform({
            position: new Vector3(-1, 0.1, -0.004),
            scale: new Vector3(0.1, 0.1, 0.1),
            rotation: Quaternion.Euler(0, 0, 0)
        }))

        this.InformationContentText = new TextShape(this.productData.metadata.exchangePolicy.label+"\n\nMin. "+this.productData.disputePeriodDuration/(60*60*24) +" days\n\nMin. "+Math.round(this.productData.disputeResolver.escalationResponsePeriod/(60*60*24)) +" days\n\nLicense Agreement v1\n\nCommerce Agreement v1")
        ...
        this.InformationContent.addComponent(new Transform({
            position: new Vector3(1, 0.1, -0.004),
            scale: new Vector3(0.1, 0.1, 0.1),
            rotation: Quaternion.Euler(0, 0, 0)
        }))
        ...
        for(let i:number = 0; i<5; i++){
            new Separator(this.parent, new Transform({
        ...
        this.LicenseClickBox.addComponent(new Transform({
            position: new Vector3(0.72,0,-0.005),
            scale: new Vector3(0.6, 0.1, 1)
        }))

        this.LicenseClickBox.addComponent(Kiosk.alphaMat as Material)
        this.LicenseClickBox.addComponent(new OnPointerDown(() => {
            
            if(this.kiosk.currentVariation!=undefined){
                openExternalURL("https://bosonapp.io/#/license/"+this.kiosk.currentVariation.offerUUID)
            } else {
                openExternalURL("https://bosonapp.io/#/license/"+this.productData.metadata.uuid)
            }
        ...
        this.CommerceClickBox.addComponent(new Transform({
            position: new Vector3(0.72,-0.12,-0.005),
            scale: new Vector3(0.6, 0.1, 1)
        }))

        this.CommerceClickBox.addComponent(Kiosk.alphaMat as Material)
        this.CommerceClickBox.addComponent(new OnPointerDown(() => {
            openExternalURL("https://bosonapp.io/#/contractualAgreement/"+this.kiosk.currentOfferID)
        }, {
