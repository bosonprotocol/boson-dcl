...
async function checkL2(
  ...
  try {
    ...
    const contract: any = await new eth.ContractFactory(
      specifiedRequestManager,
      abi
    ).at(contractId);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
function urnMatchesContractAndBlock(
  urn: string,
  contractAndBlock: string
): boolean {
...
async function checkWearables(
  ...
  for (
    ...
    if (
      urnMatchesContractAndBlock(
        wearableUrn.toLowerCase(),
        contractId.toLowerCase() + ":" + tokenId.toLowerCase()
      )
    ) {
      rtn += 1;
    }
